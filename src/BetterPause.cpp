...
namespace {
class PauseButtonScrollLayer : public geode::ScrollLayer {
protected:
	PauseButtonScrollLayer(cocos2d::CCRect const& rect)
	  : geode::ScrollLayer(rect, true, true) {}
...
public:
	static PauseButtonScrollLayer* create(cocos2d::CCSize const& size) {
		auto ret = new PauseButtonScrollLayer({ 0.f, 0.f, size.width, size.height });
		ret->autorelease();
		return ret;
	}
};

class PauseButtonRowMenu : public cocos2d::CCMenu {
protected:
	void registerWithTouchDispatcher() override {
		cocos2d::CCTouchDispatcher::get()->addTargetedDelegate(this, 10, true);
	}

public:
	static PauseButtonRowMenu* create() {
		auto ret = new PauseButtonRowMenu();
		if (ret->init()) {
			ret->autorelease();
			return ret;
		}
...
class PopupButtonMenu : public cocos2d::CCMenu {
protected:
	void registerWithTouchDispatcher() override {
		cocos2d::CCTouchDispatcher::get()->addTargetedDelegate(this, -501, true);
	}

public:
	static PopupButtonMenu* create() {
		auto ret = new PopupButtonMenu();
		if (ret->init()) {
			ret->autorelease();
			return ret;
		}
...
bool hasVisibleModal(cocos2d::CCNode* node, ChallengesPage* embeddedQuest) {
	if (!node || !node->isVisible()) return false;
	if (node != embeddedQuest && geode::cast::typeinfo_cast<FLAlertLayer*>(node) &&
		geode::cocos::nodeIsVisible(node)) {
		return true;
	}
	for (auto child : node->getChildrenExt<cocos2d::CCNode*>()) {
		if (hasVisibleModal(child, embeddedQuest)) return true;
	}
	return false;
}
}
...
bool BetterPause::init(PauseLayer* pauseLayer, bool isEditor, bool notBetter) {
	...
	m_pQuestMenu->setCascadeOpacityEnabled(false);
	...
	for (size_t i = 0; i < m_pQuestMenu->m_mainLayer->getChildrenCount(); i++)
	{
		...
		if (!geode::cast::typeinfo_cast<ChallengeNode*>(node) &&
			!geode::cast::typeinfo_cast<cocos2d::CCLabelBMFont*>(node) &&
			!geode::cast::typeinfo_cast<LoadingCircle*>(node)) {
			node->setVisible(false);
		}
	}
	...
	auto practiceButton = CCMenuItemSpriteExtra::create(practiceButtonImage, pauseLayer, (Utils::getplayLayerA()->m_isPracticeMode ?
		(cocos2d::SEL_MenuHandler)&PauseLayer::onNormalMode : (cocos2d::SEL_MenuHandler)&PauseLayer::onPracticeMode));
	...
	auto level = Utils::getplayLayerA()->m_level;
	auto const vanillaHasEditButton =
		pauseLayer->getChildByIDRecursive("edit-button") != nullptr ||
		pauseLayer->getChildByIDRecursive("edit-level-button") != nullptr;
	if (level->m_levelType == GJLevelType::Editor || level->m_isEditable || vanillaHasEditButton) {
	...
	m_pAttemptCurrentLevelLabel = TextArea::create(
		std::string(cocos2d::CCString::createWithFormat(attemptColorD, Utils::getplayLayerA()->m_attempts)->getCString()),
		"bigFont.fnt", 0.3f, 100.f, { 0.f, 1.f }, 100.f, false);
	...
	int totalSeconds = std::floor(Utils::getplayLayerA()->m_timePlayed);

	int minutes = totalSeconds / 60;
	int seconds = totalSeconds % 60;


	m_pTimeCurrentLevelLabel = TextArea::create(
		std::string(cocos2d::CCString::createWithFormat(timeColorD, minutes, seconds)->getCString()),
		"bigFont.fnt", 0.3f, 100.f, { 1.f, 1.f }, 100.f, false);
	...
	this->createToggleButton((cocos2d::SEL_MenuHandler)&BetterPause::onProgressBarA, Utils::shareManager()->m_showProgressBar, m_pMenuButtonsConfig, "Progress Bar", { 250.f, 90.f }, 0.3f);
	...
	//
	//this->createToggleButton((cocos2d::SEL_MenuHandler)&BetterPause::onProgressBarA, Utils::shareManager()->m_showProgressBar, m_pMenuButtonsConfig, "Progress Bar", { xPosButtonsToggles - 90.f, 140.f }, 0.25f, true);
	...
	auto songID_ = Utils::getplayLayerA()->m_level->m_songID;
	SongInfoObject* songObj = nullptr;

	if (songID_ == 0) {
		songObj = LevelTools::getSongObject(Utils::getplayLayerA()->m_level->m_audioTrack);
	}
	else {
	...
	this->m_pCustomSongWidget = CustomSongWidget::create(
		songObj, nullptr, false, false, true, songID_ == 0, false, false, 0);
...
void BetterPause::setupScrollableButtons(
	std::vector<CCMenuItemSpriteExtra*> const& externalButtons) {
	if (!m_pMenuButtons || m_pButtonsList) {
		return;
	}
	...
	m_pButtonsList = PauseButtonScrollLayer::create(viewportSize);
	m_pButtonsList->setPosition({ 15.f, 115.f });
	m_pButtonsList->setContentSize(viewportSize);
	m_pButtonsList->setTouchEnabled(true);
	m_pButtonsList->setStealingTouches(true);
	m_pButtonsList->setCancelTouchLimit(6.f);
	...
	for (auto child : m_pMenuButtons->getChildrenExt<cocos2d::CCNode*>()) {
		if (auto button = geode::cast::typeinfo_cast<CCMenuItemSpriteExtra*>(child)) {
	...
	auto const baseButtonCount = buttons.size();
	auto referenceButton = buttons.empty() ? nullptr : buttons.back();
	buttons.insert(buttons.end(), externalButtons.begin(), externalButtons.end());
	...
	auto setBranchVisible = [](cocos2d::CCNode* root, bool visible) {
		auto visit = [&](this auto&& self, cocos2d::CCNode* node) -> void {
			if (!node) return;
	...
	for (size_t index = 0; index < buttons.size(); ++index) {
		...
		if (!button || button == m_pVisibleButton) {
			continue;
		}
		...
		if (index >= baseButtonCount && referenceButton) {
			...
			if (normalImage && referenceImage) {
				auto const currentBox = normalImage->boundingBox();
				auto const referenceBox = referenceImage->boundingBox();
				if (currentBox.size.width > 0.f && currentBox.size.height > 0.f) {
					normalImage->setScaleX(
						normalImage->getScaleX() * referenceBox.size.width / currentBox.size.width * .9f
					);
					normalImage->setScaleY(
						normalImage->getScaleY() * referenceBox.size.height / currentBox.size.height * .9f
					);
				}
				normalImage->setPosition(referenceImage->getPosition());
			}
			button->setScale(1.f);
			button->setContentSize(referenceButton->getContentSize());
		}

		auto const height = std::clamp(button->boundingBox().size.height, 34.f, 48.f);
		auto row = cocos2d::CCNode::create();
		row->setContentSize({ viewportSize.width, height });

		auto rowMenu = PauseButtonRowMenu::create();
		rowMenu->setPosition({ 0.f, 0.f });
		row->addChild(rowMenu);
		m_scrollRowMenus.push_back(rowMenu);

		button->retain();
		button->removeFromParentAndCleanup(false);
		rowMenu->addChild(button);
		button->release();
		button->setPosition({ viewportSize.width / 2.f, height / 2.f });
		button->setVisible(true);
		button->setEnabled(true);
		if (auto normalImage = button->getNormalImage()) {
			setBranchVisible(normalImage, true);
		}
		if (auto selectedImage = button->getSelectedImage()) {
			setBranchVisible(selectedImage, false);
		}

		m_pButtonsList->m_contentLayer->addChild(row);
		rows.push_back({ row, height });
		totalHeight += height + 5.f;
	}

	totalHeight = std::max(totalHeight, viewportSize.height);
	float cursorY = totalHeight - 5.f;
	for (auto const& row : rows) {
		cursorY -= row.height;
		row.node->setPosition({ 0.f, cursorY });
		cursorY -= 5.f;
	}
	...
	m_pScrollUpSprite->setScale(0.55f);
	...
	m_pScrollDownSprite->setScale(0.55f);
...
void BetterPause::scrollWheel(float x, float y) {
	cocos2d::CCLayer::scrollWheel(x, y);
	if (m_pButtonsList) {
...
void BetterPause::update(float dt) {
	...
	updateScrollableInputState();
	updateScrollButtons();
}

void BetterPause::updateScrollableInputState() {
	if (!m_pButtonsList) return;
	auto scene = cocos2d::CCDirector::get()->getRunningScene();
	auto const enabled = !hasVisibleModal(scene, m_pQuestMenu);
	if (enabled == m_scrollInputEnabled) return;

	m_scrollInputEnabled = enabled;
	m_pButtonsList->setTouchEnabled(enabled);
	for (auto menu : m_scrollRowMenus) {
		if (menu) menu->setTouchEnabled(enabled);
	}
}
...
void BetterPause::onHide(cocos2d::CCObject* pSender) {
	...
	if (this->m_pIsHide) {
		for (size_t i = 0; i < pauseLayer->getChildrenCount(); i++)
		{
			if (!geode::cast::typeinfo_cast<BetterPause*>(pauseLayer->getChildren()->objectAtIndex(i))) {
	...
	if (m_pIsTextureEye) {
		...
		reinterpret_cast<cocos2d::CCSprite*>(
			m_pVisibleButton->getChildren()->objectAtIndex(0)
		)->initWithFile(eyeTexture.c_str());
	}
...
void BetterPause::createToggleButtonWithGameVariable(const char* key, cocos2d::CCMenu* menu, std::string caption, cocos2d::CCPoint pos, float size, bool twoColumns) {
	...
	toggleButton->setTag(geode::utils::numFromString<int>(key).unwrapOr(0));
...
void BetterPause::musicSliderChanged(cocos2d::CCObject* pSender) {
	...
	Utils::shareFMOD()->setBackgroundMusicVolume(valueV);
	this->m_pLabelMusicVPercentage->setString(
...
void BetterPause::sfxSliderChanged(cocos2d::CCObject* pSender) {
	...
	Utils::shareFMOD()->setEffectsVolume(valueV);
	this->m_pLabelSFXVPercentage->setString(
		cocos2d::CCString::createWithFormat("%i%%", static_cast<int>(valueV * 100))->getCString()
	);
}
...
void BetterPause::onInfoLevelOpen(cocos2d::CCObject* pSender) {
	...
	std::string SongID = Utils::returnZeroBool(Utils::getplayLayerA()->m_level->m_songID) ? "-" : std::to_string(Utils::getplayLayerA()->m_level->m_songID);
	...
	std::string contentStream = "<cg>Total Attempts:</c> " + std::to_string(static_cast<int>(Utils::getplayLayerA()->m_level->m_attempts)) +
		"\n<cj>Total Jumps:</c> " + std::to_string(static_cast<int>(Utils::getplayLayerA()->m_level->m_jumps)) +
		"\n<cy>Creator:</c> " + creatorname +
		"\n<cl>ID:</c> " + std::to_string(static_cast<int>(Utils::getplayLayerA()->m_level->m_levelID)) +
		"\n<cr>Song ID:</c> " + SongID +
		"\n<co>Objects Total:</c> " + ObjectCount +
		"\n<cl>Coins:</c> " + std::to_string(Utils::getplayLayerA()->m_level->m_coins);

	return FLAlertLayer::create(nullptr, "Level Stats", contentStream, "OK", nullptr, 300.f)->show();
...
void BetterPause::onAutoRetryA(cocos2d::CCObject* pSender) {
	auto manager = Utils::shareManager();
	manager->setGameVariable("0026", !manager->getGameVariable("0026"));
}

void BetterPause::onAutoCheckpointsA(cocos2d::CCObject* pSender) {
	auto manager = Utils::shareManager();
	manager->setGameVariable("0027", !manager->getGameVariable("0027"));
}
...
void BetterPause::updatePercentageObjects() {
	...
	auto manager = Utils::shareManager();
	auto const showProgressBar = manager->m_showProgressBar;

	if (playLayer->m_progressBar) {
		playLayer->m_progressBar->setVisible(showProgressBar);
	}
	if (playLayer->m_progressFill) {
		playLayer->m_progressFill->setVisible(showProgressBar);
	}

	if (playLayer->m_percentageLabel) {
		playLayer->m_percentageLabel->setVisible(manager->getGameVariable("0040"));
		if (showProgressBar && playLayer->m_progressBar) {
			auto const barBox = playLayer->m_progressBar->boundingBox();
			playLayer->m_percentageLabel->setAnchorPoint({ 0.f, 0.5f });
			playLayer->m_percentageLabel->setPosition({
				barBox.getMaxX() + 8.f,
				playLayer->m_progressBar->getPositionY()
			});
		}
		else {
			playLayer->m_percentageLabel->setAnchorPoint({ 0.5f, 0.5f });
			playLayer->m_percentageLabel->setPosition({
				Utils::winSize().width / 2.f,
				Utils::winSize().height - 8.f
			});
		}
	}
}
...
void SetVolumenPopup::onSet(cocos2d::CCObject* pSender) {
	int numberAs = geode::utils::numFromString<int>(this->m_pInputTextValue->getString())
		.unwrapOr(static_cast<int>(this->m_pSliderRef->getValue() * 100.f));
	numberAs = std::min(100, std::max(0, numberAs));
	this->m_pSliderRef->setValue(static_cast<float>(numberAs) / 100.f);
	this->m_pSliderRef->m_touchLogic->m_thumb->activate();
...
void SetVolumenPopup::onClose(cocos2d::CCObject*) {
	this->keyBackClicked();
}
...
bool SetVolumenPopup::init(Slider* m_pSliderRef) {
	...
	m_mainLayer->setPosition({ Utils::winSize().width / 2.f, Utils::winSize().height / 2.f });
	...
	m_buttonMenu = PopupButtonMenu::create();
	...
	m_pButtonsMenu = PopupButtonMenu::create();
...
bool MoreOptionsPauseLayer::init() {
	...
	m_menuButtons = PopupButtonMenu::create();
...
void MoreOptionsPauseLayer::keyBackClicked() {
	...
	Utils::shareDirectorA()->getRunningScene()->addChild(PauseLayer::create(false));
	FLAlertLayer::keyBackClicked();
}
...
PauseSettingsLayer* PauseSettingsLayer::create() {
	...
	if (node && node->init("Pause Settings", 220.0f, false)) {
		node->autorelease();
	}
	else {
		CC_SAFE_DELETE(node);
...
bool SelectQuickSettings::init() {
	...
	m_buttonMenu = PopupButtonMenu::create();
	m_buttonMenu->setPosition({ Utils::winSize().width / 2.f - 210.f, Utils::winSize().height / 2 + 140.f });
	...
	auto menu = PopupButtonMenu::create();
	...
	auto gjCommentBase = GJCommentListLayer::create(
		CustomVi, "", { 0, 0, 0, 255 }, 340.f, 200.f, false);
...
void SelectQuickSettings::onToggleWithGameVariable(CCObject* pSender) {
	...
	auto toggle = geode::cast::typeinfo_cast<CCMenuItemToggler*>(pSender);
	if (!toggle) return;
	auto const prospectiveCount = count + (toggle->isToggled() ? -1 : 1);
...
void SelectQuickSettings::keyBackClicked() {
	/*auto betterPause = static_cast<BetterPause*>(this->getParent());
	betterPause->pauseLayer->removeFromParentAndCleanup(true);
	Utils::shareDirectorA()->getRunningScene()->addChild(PauseLayer::create(Utils::getplayLayerA()->m_level->m_levelType == kGJLevelTypeEditor));*/
...
void GenericListCell::updateBGColor(int index) {
	if (index & 1) m_backgroundLayer->setColor(cocos2d::ccc3(0xc2, 0x72, 0x3e));
...
cocos2d::CCRect ListView::calculateNodeCoverage(cocos2d::CCArray* nodes) {
	...
	for (auto childObj : geode::cocos::CCArrayExt<cocos2d::CCObject*>(nodes)) {
		CCNode* child = geode::cast::typeinfo_cast<CCNode*>(childObj);
...
TableViewCell* ListView::getListCell(const char* key) {
	return GenericListCell::create(key, cocos2d::CCSize { this->m_width, 30.f });
}

void ListView::loadCell(TableViewCell* cell, int index) {
	auto node = geode::cast::typeinfo_cast<CCNode*>(m_entries->objectAtIndex(index));
	if (node) {
