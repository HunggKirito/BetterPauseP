...
namespace Utils {
	...
	void PlayLayer_getTestModeLabel(PlayLayer* self);
	...
	GJBaseGameLayer* getGJBaseGameLayer();
	...
	const char* getDifficultyIcon(GJGameLevel* level);
	bool isObjectInVector(const std::vector<std::pair<GameObject*, cocos2d::CCPoint>>& vec, GameObject* obj);
	bool isObjectInVector(const std::vector<std::pair<StartPosObject*, cocos2d::CCPoint>>& vec, GameObject* obj);
}
